...
## Action items
Bullet list of tasks with owner if mentioned (e.g. "- Alex: send the
...
def generate_notes(text, language="English", llm=None, log=print):
    """Meeting notes from transcript text.

    Offline-only by default: notes are generated by a local Ollama model and
    ...
    if policy.cloud_allowed():
        ...
        if key:
            try:
                ...
                if model:
                    log(f"  notes: using {model} via Groq (cloud)...")
                    return _chunked(
                        text, lambda p: _groq_generate(p, key, model),
    ...
    try:
        ...
        raise NotesUnavailable(
            "Ollama isn't running — install it (free, ollama.com) and run "
            "'ollama pull llama3.2:3b' to generate meeting notes offline"
        ) from None
    if not model:
        raise NotesUnavailable(
    ...
    try:
        return _chunked(text, lambda p: _ollama_generate(p, model),
            ...
            ValueError) as e:
        raise NotesUnavailable(f"Ollama failed: {e}") from e
